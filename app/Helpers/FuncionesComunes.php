...
class FuncionesComunes
{
    ...
    public static function serearNumero($cantidad, $numero){
        $digitos = (int) log10($numero);
        $n = $cantidad-$digitos;
        $ceros = "";
        for($i=1;$i<$n;$i++){
            $ceros .= "0";
    ...
    public function getLastDayByGestionAndMes($gestion, $mes) {
        return date("d", (mktime(0, 0, 0, ((int) $mes) + 1, 1, $gestion) - 1));
    }

    public function calcularEdad($fecNac, $fecCal) {
        
        if (!empty($fecNac) && $this->validationDate($fecNac)) {
            list($Y, $m, $d) = explode("-", $fecNac);
            list($anio, $mes, $dia) = explode("-", $fecCal);
            if($mes . $dia < $m . $d){
                return ($anio - $Y - 1);
            }
            return( $anio - $Y );
    ...
    public function lastDayOfMonth($mes, $gestion) {
        return date("d", (mktime(0, 0, 0, $mes + 1, 1, $gestion) - 1));
    }
    
    public static function validationDate($fecha) {
        $aux = date_parse($fecha);
        return checkdate($aux['month'], $aux['day'], $aux['year']);
    }

    public static function getFechaByDiaMesAnio($dia, $mes, $anio){
    ...
    public static function getMesLiteral($mes){
    ...
    public static function fecha_literal($Fecha, $Formato = 4) {
    ...
    public static function getPartCodCtr(){
        $c = "";
        $tipo = rand(1,2);
        if($tipo==1){
            $car = chr(rand(65,90));;
    ...
    public function convertir($numero, $moneda){
                ...
                if ($numf == "UNO")
                        {
                            ...
                            //$Ps = " PESO "; //Cambiado por Ivan
                            $Ps = " ";
                        }
                        else
                        {
                            //$Ps = " PESOS ";  //Cambiado por Ivan
                            $Ps = " ";
                        }
            $TextEnd = $numf;
            $TextEnd .= $Ps;
        ...
        $TextEnd .= $tempnum[1] ;
        //$TextEnd .= "/100 M.N."; //Cambiado por Ivan
        $TextEnd .= "/100 ".$moneda; 
    return $TextEnd;
    }
}

        ...
        <div class="panel panel-inverse">
            ...
            <div class="row">
                    <div class="col-md-9 text-center" style="">
                        <h2 style="margin-bottom:0">Administración de Historial Clínico</h2>
            ...
            <div class="panel-body">
                <table id="data-table" class="table table-striped table-bordered">
                    ...
                    <tbody id="tbody">
                        @if(!empty($historial_clinico))
                        @foreach($historial_clinico as $key=>$value)
                        <tr class="" id="tabla-tr-1">
                            <td></td>
                            <td class="td-acciones">
                                <a class="btn btn-xs btn-inverse" href="{{url('historial-clinico-ver/'.$value->id)}}" target="blank"><span class="fa fa-eye"></span> Ver</a>
                                @if(!auth()->user()->hasRole('secretaria'))
                                <a class="btn btn-xs btn-info" href="{{url('historial-clinico-modificar/'.$value->id)}}" title="" target="blank"><span class="fa fa-pencil-square-o"></span> Editar</a>
                                @endif
                                <a class="btn btn-xs btn-info" href="{{url('historial-clinico-reporte/'.$value->id)}}" title="" target="blank"><span class="fa fa-print"></span> PDF</a>
                                @if(!auth()->user()->hasRole('secretaria'))
                            ...
                            <td >{{$value->materno}}</td>
                            <td >{{$value->ci}}</td>
                            <td >{{substr($value->motivo_consulta, 0, 30)}}</td>
                            <td >{{substr($value->dolencia_actual,0,30)}}</td>
                            <td >{{substr($value->antecedente_familiar,0,30)}}</td>
                            <td >{{substr($value->antecedente_personal,0,30)}}</td>
                            <td >{{substr($value->diagnostico,0,30)}}</td>
...
<script>
    ...
    function eliminar() {
        ...
        window.location.href = "{{url('historial-clinico-eliminar')}}/"+id;
    }

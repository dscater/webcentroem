...
@section('content_contenido')
                ...
                <div class="row">
                    ...
                    <div class="col-md-4" style="padding-left:20px">
                        ...
                        <div class="div-create-nombre">
                            <input name="fecha_busqueda" id="fecha_busqueda" value="{{ date('Y-m-d') }}"
                                class="app-form-control form-control  fadeInLeft animated" id="input-nombre" type="date">
                        </div>
                        <div class="app-alert alert alert-danger" style="display:none" id="alert-especialidad">El campo es
                            requerido</div>

                    </div>
                    <div class="col-md-4">
                        <button style="margin-top:10px" class="btn btn-success" onclick="buscarPorFecha()">Buscar por
                            fecha</button>
                    </div>
                </div>
                                    ...
                                    <tr class="" id="tabla-tr-1">
                                        ...
                                        <td class="td-acciones">
                                            <a class="btn btn-xs btn-inverse"
                                                href="{{ url('cita-medica-ver/' . $value->id) }}" target="blank"><span
                                                    class="fa fa-eye"></span> Ver</a>

                                            @if (!auth()->user()->hasRole('paciente'))
                                                <a class="btn btn-xs btn-info"
                                                    href="{{ url('cita-medica-modificar/' . $value->id) }}" title=""
                                            ...
                                            @if (
                                                ...
                                                <button class="btn btn-xs btn-success"
                                                    onclick="clickAtender({{ $value->id }})" title="">
                                                    Atender</Button>
                                            @endif
                                            @if (!auth()->user()->hasRole('paciente'))
                                                <button class="btn btn-xs btn-danger"
                                                    onclick="clickEliminar({{ $value->id }})" title=""><span
                                                        class="fa fa-trash"></span> Eliminar</Button>
                                            @endif
                                            @if (auth()->user()->hasRole('secretaria') ||
                                                    auth()->user()->hasRole('doctor'))
                                                @if ($value->estado != 'CANCELADO' && $value->estado != 'ATENDIDO')
                                                    <button class="btn btn-xs btn-warning"
                                                        onclick="clickCancelar({{ $value->id }})" title="">
                                                        Cancelar
                                                        Cita</Button>
                                                @endif
                                            @endif
    ...
    <!-- Button trigger modal -->
    <input id="input-modal-atender" type="hidden" class="btn btn-primary btn-lg" data-toggle="modal"
        data-target="#modal-atender">
    ...
    <!-- Button trigger modal -->
    <input id="input-modal-cancelar" type="hidden" class="btn btn-primary btn-lg" data-toggle="modal"
    ...
    <div class="modal fade" id="modal-cancelar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="width:40%">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">No</button>
                    <button type="button" class="btn btn-danger" onclick="cancelar()">Si</button>
                </div>
            </div>
        </div>
    </div>
...
@section('script_3')
    ...
    <script>
        ...
        function clickCancelar(id) {
            $("#cancelar-id").val(id)
            $("#input-modal-cancelar").trigger("click");
        }

        function cancelar() {
            var id = $("#cancelar-id").val();
            window.location.href = "{{ url('cita-medica-cancelar') }}/" + id;
        }

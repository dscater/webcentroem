        ...
        <div class="panel panel-inverse">
            ...
            <div class="row">
                    <div class="col-md-9 text-center" style="">
                        <h2 style="margin-bottom:0">Administración de Pacientes</h2>
                ...
                <table id="data-table" class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th id="th-id_user">Tipo de Usuario</th>
                            ...
                            <th id="th-ci">CI</th>
                            <th id="th-genero">Sexo</th>
                            <th id="th-edad">Edad</th>
                            <th id="th-estado_civil">Estado civil</th>
                            <th id="th-familiar_responsable">Familiar responsable</th>
                            <th id="th-fecha_nacimiento">Fecha nacimiento</th>
                            <th id="th-domicilio">Domicilio</th>
                            <th id="th-telefono">Teléfono</th>
                            <th id="th-email">Email</th>
                            <th id="th-foto">Foto</th>
                            <th id="th-lugar_nacimiento">Lugar nacimiento</th>
                    ...
                    <tbody id="tbody">
                        ...
                        <tr class="" id="tabla-tr-1">
                            ...
                            <td >{{$value->familiar_responsable}}</td>
                            <td >{{date("d-m-Y", strtotime($value->fecha_nacimiento))}}</td>
                            <td >{{$value->domicilio}}</td>
                            <td >{{$value->telefono}}</td>
                            <td >{{$value->email}}</td>
                            <?php $foto = (!empty($value->foto))? url('fotoPersona/'.$value->foto):url("img/user-avatar.png")?>
                            <td ><img src="{{$foto}}" width="80"/></td>
                            <td >{{$value->lugar_nacimiento}}</td>
                            <td >{{date("d-m-Y", strtotime($value->created_at))}}</td>
                            <td class="td-acciones">
                                <a class="btn btn-xs btn-inverse" href="{{url('paciente-ver/'.$value->id)}}" target="blank"><span class="fa fa-eye"></span> Ver</a>
                                <a class="btn btn-xs btn-info" href="{{url('paciente-modificar/'.$value->id)}}" title="" target="blank"><span class="fa fa-pencil-square-o"></span> Editar</a>
                                <a class="btn btn-xs btn-info" href="{{url('paciente-reporte/'.$value->id)}}" title="" target="blank"><span class="fa fa-print"></span> PDF</a>
...
<script>
    ...
    function eliminar() {
        ...
        window.location.href = "{{url('paciente-eliminar')}}/"+id;
    }
